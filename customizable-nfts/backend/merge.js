...
import {
  ...
  import { jsonResponseToTokenUri, stringToMap, intToHexString, replaceTokenCurrentId, pinataToHTTPUrl } from './converters.js';
  dotenv.config();
...
//   };
  ...
  const listOfTuplesResponseToList = (tupleResponse) => {
    ...
    tupleList.forEach((x) => {
      idLists.push({ degenId: x.value['degen-id'].value, degenType:x.value['degen-type'].value, address: x.value.member.value });
    });
    return idLists;
  };
