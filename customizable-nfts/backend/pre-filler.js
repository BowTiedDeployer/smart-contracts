...
export const mintNMiami = async (n, walletAddress) => {
  ...
  // console.log('//');
  for (let i = 0; i < n; i++) {
    console.log(`before mintMiami${i}`);
...
const prefillMintThingsOut = async () => {
  ...
  await instantiateAllAccountsNonce();
  // max 25 tx per block, else server call throws error
...
const prefillAddToQueueThingsOut = async () => {
  ...
  await instantiateAllAccountsNonce();
  // // SWAP
  // console.log('degenIndexSWAP', degenIndex, 'componentindexSwap', componentIndex);
  // const swapNr = 4;
